...
Expected: the vault track surfaces the active decision (and skips the superseded one), returns a contradiction flag naming the decision slug and both claims, and Guild surfaces it in Phase 1 before the resolver and fan-out — not silently proceeding. *(Also verified against the real acme-site vault, whose `2026-07-08-guild-deprecated-and-comingsoon-retired` decision carries the same ruling.)*
